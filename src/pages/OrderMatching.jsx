...
import { Search, Trash2, Send, Shuffle, Plus, X, QrCode, CheckCircle } from 'lucide-react';
...
const OrderMatching = () => {
  ...
  const [activeFilters, setActiveFilters] = useState({ code: '', tableSearch: '' });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        const esimRows = (rows || [])
          .filter(o => o.type === 'eSIM')
          .map((o, idx) => ({
            id: o.id || String(idx + 1),
            serial: idx + 1,
            code: o.orderId || o.id || '',
            time: o.date || '',
            status: o.qrcode ? 'Đã nhận QR' : (o.status === 'Success' ? 'Chờ gửi' : (o.status || 'Chờ xử lý')),
            email: o.email || '',
            qrcode: o.qrcode || null,
            qrcodeContent: o.qrcodeContent || null,
            pin1: o.pin1 || null,
            puk1: o.puk1 || null,
            cfCode: o.cfCode || null,
            apnExplain: o.apnExplain || null,
          }));
        setOrders(esimRows);
      } catch (err) {
        console.error('Load eSIM orders failed:', err);
      } finally {
  ...
  // Lắng nghe callback QR từ WorldMove API (khi dùng "Đặt mua & Đổi eSIM siêu tốc")
  useEffect(() => {
    const handleCallback = (e) => {
      if (e.detail.callbackType === 'orderRedeem') {
        ...
        setActiveEsimDetail({ orderId, ...esimInfo });
      }
    };
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    setActiveFilters({ code: orderCode, tableSearch });
  };

  const handleClear = () => {
    setOrderCode(''); setStartDate(''); setEndDate(''); setTableSearch('');
    setActiveFilters({ code: '', tableSearch: '' });
  };
  ...
  const handleSend = (id) => {
    ...
    if (!order.qrcodeContent && !order.qrcode) {
      alert('Đơn này chưa có mã QR. Vui lòng chờ callback từ WorldMove trước khi gửi.');
      return;
    }

    // Gửi qua mailto — mở email client của user
    const subject = encodeURIComponent(`[WorldMove eSIM] Mã kích hoạt eSIM - Đơn ${order.code}`);
    const body = encodeURIComponent(
      `Xin chào,\n\nDưới đây là thông tin eSIM của bạn:\n\n` +
      `📋 Mã đơn hàng: ${order.code}\n\n` +
      `📱 Mã cài đặt thủ công (LPA String):\n${order.qrcodeContent || 'N/A'}\n\n` +
      (order.pin1 ? `🔐 PIN1: ${order.pin1}\n` : '') +
      (order.puk1 ? `🔐 PUK1: ${order.puk1}\n` : '') +
      (order.cfCode ? `✅ Confirmation Code: ${order.cfCode}\n` : '') +
      (order.apnExplain ? `📡 APN: ${order.apnExplain}\n` : '') +
      `\nTrân trọng.`
    );
  ...
  const handleInstantSubmit = async (e) => {
    ...
    try {
      ...
      if (res.code === 0 && res.orderId) {
        ...
        alert(`Đặt mua eSIM siêu tốc thành công!\nMã giao dịch: ${res.orderId}\nVui lòng chờ Webhook callback trả về mã QR.`);
      } else {
  ...
  const dateFiltered = filteredOrders.filter(item => {
    ...
    <div className="page-wrapper">
        ...
        <div className="card-body">
          ...
          <form onSubmit={handleSearch} style={{ display: 'flex', flexDirection: 'column', gap: '15px', marginBottom: '25px', borderBottom: '1px solid var(--border-color)', paddingBottom: '20px' }}>
            <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap' }}>
              ...
              <div style={{ flex: '1 1 200px', display: 'flex', flexDirection: 'column', gap: '5px' }}>
                ...
                <input type="date" className="search-input" style={{ width: '100%' }} value={startDate} onChange={(e) => setStartDate(e.target.value)} />
              </div>
              <div style={{ flex: '1 1 200px', display: 'flex', flexDirection: 'column', gap: '5px' }}>
                <label>Ngày kết thúc</label>
                <input type="date" className="search-input" style={{ width: '100%' }} value={endDate} onChange={(e) => setEndDate(e.target.value)} />
              </div>
            </div>
          ...
          {loading && (
            <div style={{ textAlign: 'center', padding: '20px', color: 'var(--text-muted)', fontSize: '13px' }}>
              Đang tải đơn hàng eSIM...
            </div>
          )}

          {!loading && !firebaseEnabled && (
            <div style={{ marginBottom: '12px', padding: '10px 14px', background: 'rgba(240,173,78,0.1)', border: '1px solid rgba(240,173,78,0.3)', borderRadius: '6px', fontSize: '12px', color: 'var(--orange-primary)' }}>
              Firebase chưa kết nối — danh sách đơn hàng trống. Đặt eSIM siêu tốc hoặc kiểm tra cấu hình Firebase.
            </div>
          )}
            ...
            <table className="notices-table">
              ...
              <tbody>
                {dateFiltered.length > 0 ? (
                  dateFiltered.map((row, idx) => (
                    <tr key={row.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                      <td className="notice-cell" style={{ padding: '12px 15px', textAlign: 'center' }}>{idx + 1}</td>
                      <td className="notice-cell" style={{ padding: '12px 15px', fontWeight: '500', fontFamily: 'monospace', color: 'var(--teal-primary)' }}>
                        <span
                          onClick={() => row.qrcode && setActiveEsimDetail({ orderId: row.code, qrcode: row.qrcode, qrcodeContent: row.qrcodeContent, pin1: row.pin1 || '1111', puk1: row.puk1 || '33334444', cfCode: row.cfCode || '', apnExplain: row.apnExplain || '' })}
                      ...
                      <td className="notice-cell" style={{ padding: '12px 15px', textAlign: 'center' }}>
                        <button
                          ...
                          title={row.qrcodeContent ? 'Gửi mã LPA qua email' : 'Chưa có mã QR'}
                        >
                          <Send size={10} />
                          <span>Gửi đi</span>
                        </button>
                  ...
                  <tr>
                    <td colSpan="6" className="notice-cell" style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '30px' }}>
                      {loading ? 'Đang tải...' : 'Không có đơn hàng eSIM nào.'}
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      {showInstantModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10000 }}>
          <div style={{ backgroundColor: '#1b222a', border: '1px solid var(--border-color)', borderRadius: '10px', width: '450px', padding: '20px', boxShadow: '0 10px 25px rgba(0,0,0,0.5)', display: 'flex', flexDirection: 'column', gap: '15px' }}>
            ...
            <form onSubmit={handleInstantSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                ...
                <select className="entries-select" style={{ width: '100%', height: '30px' }} value={productId} onChange={(e) => setProductId(e.target.value)}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                ...
                <input type="number" min="1" className="search-input" value={qty} onChange={(e) => setQty(e.target.value)} style={{ width: '100%' }} />
              </div>
      ...
      {activeEsimDetail && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10001 }}>
          <div style={{ backgroundColor: '#1b222a', border: '2px solid var(--teal-primary)', borderRadius: '12px', width: '500px', padding: '25px', boxShadow: '0 15px 30px rgba(0,0,0,0.7)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '15px', position: 'relative' }}>
            <X size={20} style={{ position: 'absolute', right: '15px', top: '15px', cursor: 'pointer', color: '#9ca3af' }} onClick={() => setActiveEsimDetail(null)} />
            ...
            {activeEsimDetail.qrcode ? (
              <div style={{ padding: '10px', backgroundColor: '#fff', borderRadius: '8px', marginTop: '10px', boxShadow: '0 0 10px rgba(32, 158, 145, 0.4)' }}>
                <img src={activeEsimDetail.qrcode} alt="eSIM QR Code" style={{ width: '160px', height: '160px', display: 'block' }} />
              </div>
            ) : (
            ...
            <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '4px', marginTop: '5px' }}>
              ...
              <div style={{ backgroundColor: '#11151b', border: '1px solid var(--border-color)', borderRadius: '5px', padding: '8px 12px', fontFamily: 'monospace', fontSize: '11px', color: 'var(--teal-primary)', wordBreak: 'break-all', textAlign: 'center' }}>
            ...
            <div style={{ width: '100%', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', fontSize: '12px', backgroundColor: '#11151b', padding: '12px', borderRadius: '6px', border: '1px solid var(--border-color)' }}>
              <div><span style={{ color: 'var(--text-muted)' }}>Mã PIN1:</span> <strong style={{ color: '#fff' }}>{activeEsimDetail.pin1 || '1111'}</strong></div>
              <div><span style={{ color: 'var(--text-muted)' }}>Mã PUK1:</span> <strong style={{ color: '#fff' }}>{activeEsimDetail.puk1 || '33334444'}</strong></div>
              <div style={{ gridColumn: 'span 2' }}><span style={{ color: 'var(--text-muted)' }}>Cấu hình APN:</span> <strong style={{ color: 'var(--orange-primary)' }}>{activeEsimDetail.apnExplain || 'worldmove.com'}</strong></div>
              <div style={{ gridColumn: 'span 2' }}><span style={{ color: 'var(--text-muted)' }}>Mã xác nhận:</span> <strong style={{ color: '#fff' }}>{activeEsimDetail.cfCode || ''}</strong></div>
            </div>
            <button className="btn btn-teal" style={{ marginTop: '10px', width: '100%', justifyContent: 'center', height: '32px' }} onClick={() => setActiveEsimDetail(null)}>
              Hoàn thành
            </button>
          </div>
        </div>
